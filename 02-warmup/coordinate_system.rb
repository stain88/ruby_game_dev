  ...
  def initialize(width, height)
    ...
    (0..width).step(50) do |x|
      ...
      (0..height).step(50) do |y|
        img = Gosu::Image.from_text("#{x}:#{y}", 15, { :font => Gosu.default_font_name })
        @images[x][y] = img
      end
    end
  end
...
class Camera
  ...
  def draw_crosshair
    $window.draw_line(@x - 10, @y, Gosu::Color::YELLOW, @x + 10, @y, Gosu::Color::YELLOW, 100)
    $window.draw_line(@x, @y - 10, Gosu::Color::YELLOW, @x, @y + 10, Gosu::Color::YELLOW, 100)
  end
end
...
class GameWindow < Gosu::Window
  ...
  def draw
    ...
    info_img = Gosu::Image.from_text(info, 30, { :font => Gosu.default_font_name })
